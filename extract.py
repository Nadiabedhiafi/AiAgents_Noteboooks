...
# Pattern matching: (A1), (A2), (B1), (C3)… followed by an optional title

SECTION_PATTERN = re.compile(r’(([A-Z]\d+))\s*[-–—:•]?\s*(.*)’, re.IGNORECASE)
...
def build_analysis_prompt(section_key: str, entries: list[dict]) -> str:
“”“Build the prompt sent to Claude to analyze a given section type.”””
blocks = []
for i, entry in enumerate(entries, 1):
blocks.append(
f”— Source {i}: {entry[‘source’]} —\n”
f”Title: {entry[‘title’]}\n”
f”Content:\n{entry[‘content’]}\n”
)
...
return f"""You are analyzing ({section_key}) sections extracted from multiple credit memos of different companies.
```

Here is the content of these sections:

{sections_text}

Respond with valid JSON ONLY — no text or Markdown fences around it — using exactly these fields:
...
“”“Send a section type’s contents to Claude and return the analysis.”””
prompt = build_analysis_prompt(section_key, entries)
...
for key in sorted_keys:
    ...
    print(f"  🔍 Analyzing section ({key}) — {len(entries)} occurrence(s)...")
...
headers = ["Section", "Occurrences", "PDF Sources", "Description", "Common Points", "Questions"]
widths   = [10,        14,             40,            60,            16,              14]
...
for i, r in enumerate(results, 2):
    ...
    _cell(ws, i, 2, r.get("occurrences", 0), bg=bg, align_h="center")
    _cell(ws, i, 3, ", ".join(r.get("sources", [])), bg=bg)
    _cell(ws, i, 4, r.get("description", ""), bg=bg)
    _cell(ws, i, 5, len(r.get("common_points", [])), bg=bg, align_h="center")
    _cell(ws, i, 6, len(r.get("suggested_questions", [])), bg=bg, align_h="center")
...
for r in results:
    ...
    _merge_row(ws, row, 1, 2,
               value=f"  Section ({key})   —   {occ} occurrence(s)",
